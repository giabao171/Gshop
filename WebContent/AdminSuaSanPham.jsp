...
<body>
	<nav class="navbar navbar-inverse">
		<div class="container-fluid">
			...
			<ul class="nav navbar-nav">
				...
				<li class="active"><a href="AdminQuanLySanPhamController">Quản lý sản phẩm</a></li>
	...
	<div class="box-con">
		<div class="container">
			<form method="post" action="AdminSuaSanPhamController" enctype="multipart/form-data">
				<div class="box">
				<c:if test="${mohinhitem!= null }">
					<div class="form-row">
						<div class="form-group col-md-6">
							<label for="inputEmail4">Mã sản phẩm</label> <input type="text"
								name="txtmamohinh" name="" class="form-control" id="inputEmail4" value="${mohinhitem.getMamohinh() }">
						</div>
						<div class="form-group col-md-6">
							<label for="inputPassword4">Tên sản phẩm</label> <input
								name="txttenmohinh" type="text" class="form-control"
								id="inputPassword4" value="${mohinhitem.getTenmohinh() }">
						</div>
					</div>
					...
					<div class="form-row">
						<div class="form-group col-md-6">
							<label for="inputEmail4">Số lượng</label> <input type="text"
								name="txtsoluong" name="" class="form-control" id="inputEmail4" value="${mohinhitem.getSoluong() }">
						</div>

						<div class="form-group col-md-6">
							<label for="inputPassword4">Giá</label> <input type="text"
								name="txtgia" name="" class="form-control" id="inputPassword4" value="${mohinhitem.getGia() }">
						</div>
					</div>
					...
					<select id="inputState"
							...
							<c:forEach items="${dsloai }" var="s">
								<c:if test="${sachitem.getMaloai() eq s.getMaloai() }">
									<option value="${sachitem.getMaloai()}" selected="selected">
										${s.getTenloai() }</option>
								</c:if>
							</c:forEach>
							...
							<c:forEach items="${dsloai }" var="s">
								<c:choose>
									<c:when test="${sachitem.getMaloai() eq s.getMaloai() }">
										<option value="${sachitem.getMaloai()}" hidden="" >
											${s.getTenloai() }</option>
									</c:when>
									<c:otherwise>
						...
						<div class="form-group col-md-6">
							<label for="inputPassword4">Nhà sản xuất</label> <input
								...
								id="inputPassword4" value="${mohinhitem.getNhasanxuat() }">
						</div>
					</div>
					...
					<div class="form-row">
						<div class="form-group col-md-6">
							<label for="inputEmail4">Id review</label> <input type="text"
								name="txtidreview" class="form-control" id="inputEmail4" value="${mohinhitem.getIdreview() }">
						</div>
						<div class="form-group col-md-6">
							<label for="inputPassword4">Ảnh</label> <input type="file" value="${mohinhitem.getAnh() }"
	...
	<c:if test="${ebox == '1' }">
		<div class="overlay">
			<div class="er-box">
				<h4>Lỗi! Không thể sửa sản phẩm này</h4>
				<btn class="btn btn-success"> <a
					href="AdminSuaSanPhamController?noterr=1">OK</a> </btn>
			</div>
		</div>
	</c:if>
